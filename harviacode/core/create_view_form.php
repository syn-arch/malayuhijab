...
foreach ($non_pk as $row) {
    ...
    if ($join) {
        ...
        if (in_array(str_replace('id_', '', $row['column_name']), $table_join)) {
            foreach ($table_join as $tb) {
                if ($row['column_name'] == 'id_' . $tb) {
                    $string .= "\n\t <div class=\"form-group <?php if(form_error('" . $row['column_name'] . "')) echo 'has-error'?> \">
                                                    <label for=\"" . $row["column_name"] . "\">Nama " . ucfirst($tb) . "</label>
                                                    <select class=\"form-control select2\" name=\"" . $row['column_name'] . "\">
                                                        <option>-- Silahkan Pilih --</option>
                                                        <?php foreach(\$$tb as \$row) : ?>
                                                            <option value=\"<?php echo \$row->id_$tb ?>\"
                                                            <?php echo \$row->id_$tb == $" . $row['column_name'] . " ? 'selected' : ''  ?>
                                                            > <?php echo \$row->nama_$tb ?> </option>
                                                        <?php endforeach; ?>
                                                    </select>
