...
class " . $m . " extends CI_Model
{
    ...
    if ($jenis_tabel <> 'reguler_table') {
        ...
        function json() {
            ...
            if ($join) {
                foreach($table_join as $index => $row){
                    $string .= "\n\$this->datatables->join('$row', '".$row.".$id_table_join[$index] = $table_name.$id_table_join[$index]');";
                }
            }
    ...
    function get_all()
    {
        ...
        if ($join) {
            foreach($table_join as $index => $row){
                $string .= "\n\$this->db->join('$row', '".$row.".$id_table_join[$index] = $table_name.$id_table_join[$index]', 'left');";
            }
        }
